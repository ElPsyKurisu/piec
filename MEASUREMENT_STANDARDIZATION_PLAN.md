...
| 5 | Completed | Aligned VirtualSourcemeter Level 2 source/sense surface to accept channel=1 keyword convention across output, set_source_function, set_sense_function, set_sense_mode, set_source_voltage, set_source_current, set_voltage_compliance, set_current_compliance, configure_voltage_source, configure_current_source, quick_read, get_voltage, get_current, and get_resistance. Added get_resistance(channel=1) to base Sourcemeter. Added _normalize_arg_compatibility hook in AutoCheckMeta/Instrument/Sourcemeter to normalize supported legacy arguments before automatic parameter validation with check_params=True and False. Distinguish omitted values, explicit channel keywords, and legacy positional calls, rejecting missing values without changing state or writing commands. Preserved legacy configuration call conventions (e.g. configure_voltage_source(1.0, 0.05)). Refined MOKE test recording wrapper to reject missing values and record actual voltage after setter succeeds. Added 125 focused contract tests in tests/test_sourcemeter_contract.py. All 704 tests pass, 2 xfailed in 17.26s on Python 3.13.2. Checkpoint 6 remains next. |
| 5 follow-up | Completed | Hardened Sourcemeter argument normalizer against silent drops, duplicate arguments, and excess arguments. Preserved valid mixed positional-channel and keyword-value calls across output, numeric setters, function/mode setters, and configure methods. Translated only clearly identified legacy forms (output(bool), single positional value, 2-arg positional configure); retained standard Python argument-binding TypeError for duplicate, unknown, or excess arguments before state changes or transport writes. Added 16 focused regression tests across VirtualSourcemeter and Keithley2400 in tests/test_sourcemeter_contract.py (141 total in contract suite). Full suite: 720 passed, 2 xfailed in 20.48s on Python 3.13.2. Checkpoint 6 remains next. |
...
| 6 | Next; not started | DMM voltage-read audit. |
| 7 onward | Not started | Continue driver prerequisites, engine, and family slices. |
