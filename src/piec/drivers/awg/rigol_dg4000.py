...
class RigolDG4000(Scpi, Awg):
    ...
    def set_pulse_rise_time(self, channel=1, rise_time=2.5e-9):
         self.instrument.write(f"SOUR{channel}:FUNC:PULS:TRAN:LEAD {rise_time}")

    def set_pulse_fall_time(self, channel=1, fall_time=2.5e-9):
         self.instrument.write(f"SOUR{channel}:FUNC:PULS:TRAN:TRA {fall_time}")
         
    def set_pulse_edge_time(self, channel=1, edge_time=2.5e-9):
        # Set both
